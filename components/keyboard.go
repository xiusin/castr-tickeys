...
type keyBoardListener struct {
	...
	initLabel         func(bool) *widgets.QLabel
	blurLabels        struct {
		labels []*struct {
			timer *time.Timer
...
func InitKeyboard(app *widgets.QApplication) *keyBoardListener { //, win *widgets.QMainWindow
	keyboardListenerInstance.Do(func() {
		...
		keyboardListenerInstance.activeLabel = keyboardListenerInstance.initLabel(false)
		...
		go func() {
			s := hook.Start()
			for ev := range s {
				if strings.Contains(ev.String(), "Mouse") {
					continue
				}
				fmt.Println(ev)
				var kc string
				...
				if ev.Kind == hook.KeyUp {
					...
					func(keyChar string) {
						...
						defer keyboardListenerInstance.Unlock()
						go func() {
							p := streams.Ctx.NewPlayer()
							_, _ = p.Write(soundData)
							_ = p.Close()
						}()
